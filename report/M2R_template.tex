...
\subsection{Methods to solve CDEs}
...
CDEs are much harder to solve than ODEs since there is a control term introduced in the equation. For the general case, we don't know whether the control path $x_t$ is differentiable or not. Using powerful tools from rough path theory, one can derive an elegant result called the \textit{log-ODE method}, which approximates the CDE by an ODE containing the log-signature of the control path. The details of the log-ODE method are omitted, as the statement and proof of this result would take up too much space in this report. For interested readers, see c for details.
...
To do so, we need to employ a standard technique from numerical analysis, called \textit{cubic spline}. The only thing that we need to know is that cubic splines are continuously differentiable.
