...
\begin{figure}[htbp]
    ...
    \begin{minipage}{0.32\textwidth}
        ...
        \begin{tikzpicture}
            \begin{axis}[
                title={$1/\left(1+\mathrm{e}^{-x}\right)$},
                xlabel={$x$},
                ylabel={$y$},
                grid=major,
                width=\textwidth,
...
In this section, we provide a proof of the adjoint sensitivity method. The proof references the method from ‘A Note on the Adjoint Method for Neural ODEs’ \cite{hu2024note}. However, we adjust the choice of the Lagrange multiplier to ensure that the adjoint states adjust in the same direction as outlined in the original neural ODE paper \cite{chen2018neural}. Additionally, we used a clearer notation to illustrate the adjustment process at each evaluation point.

We know that there will be multiple evaluation points on the trajectory and that these points are obtained as the output of the model. So, each evaluation point $z(t_i)_{i=1,\dots,N}$ contributes to the loss function. Our first goal is to find the gradient of the loss function with respect to the parameter $\theta$. We can turn this into an optimisation problem:
$$
\min_{\theta} \mathcal{L}(z(t_1),\dots,z(t_{tN})),
$$
$$
...
Note that we also impose a constraint on $\lambda(t)$ such that $\lambda(t_N)=\lambda(t_N^+)=0$. To find the gradient with respect to $\theta$, we introduce a small perturbation to $\theta$ to see how it affects the cost function. Therefore, we define the perturbed parameter as
$$
...
Assuming that $\lambda(t)$ is continuous in each pair of successive time steps $[t_i,t_{i+1}]_{i=0,\dots,N-1}$, the second term yields 
\begin{align}
...
To transform the third term using the same method, we need to first get the value of $\dot{\eta}$, and from the definition of $z^*(t)$ we know $\eta(t)=\frac{\mathrm{d}z^*(t)}{\mathrm{d}\epsilon}\big\rvert_{\epsilon=0}$. Then we have
\begin{align}
...
&+\int^{t_N}_{t_0}\lambda(t)\frac{\partial f}{\partial \theta}\epsilon\zeta\mathrm{d}t+\mathcal{O}(\epsilon^2).\tag{3}
\end{align}
...
\begin{equation}\label{eq4}
\dot{a}(t)=a(t)\frac{\partial f}{\partial z}\quad\quad a^{\mathrm{initial}}(t_i)=a(t_i)+\frac{\partial\mathcal{L}}{\partial z(t_i)}, \quad i=N,N-1,\dots,1.\tag{4}
\end{equation}

and by substituting this $a(t)$ as $\lambda$ in (\ref{eq3}) we can get
$$
...
Then the gradient of the Lagrange loss with respect to $\theta$ is
$$
...
For $a_\theta(t)$, to simplify the computation, we assume that $a_\theta(t_N)=0$:
$$
a_{\theta}(t) = \frac{\mathrm{d}\mathcal{L}}{\mathrm{d} z(t)}\frac{\mathrm{d} z(t)}{\mathrm{d}\theta}=a(t)\frac{\mathrm{d} z(t)}{\mathrm{d}\theta}.
$$

Notice that the loss $\mathcal{L}$ is the same as the Lagrange loss $\mathcal{J}$. Thus, we have
\begin{align}
...
\begin{equation}\label{eq5}
    \frac{\mathrm{d}a_{\theta}(t)}{\mathrm{d}t} = -a(t)\frac{\partial f}{\partial \theta}. \tag{5}
\end{equation}
...
\begin{equation}\label{eq6}
    \frac{\mathrm{d}a_t(t)}{\mathrm{d}t} = -a(t)\frac{\partial f}{\partial t}. \tag{6}
\end{equation}

We noticed that equations (\ref{eq4}), (\ref{eq5}), (\ref{eq6}) follow a similar pattern. By adding a minus sign when defining $a_t(t)$, we ensure that equation (\ref{eq6}) aligns with the pattern of equations (\ref{eq4}), (\ref{eq5}). Accordingly, we define the augumented state $a_{\mathrm{aug}}$ and the corresponding differrential equation $f_{\mathrm{aug}}$ as follows
...
From the above CDE, we may conclude that the hidden state $Z_t$ of the recurrent neural network can be continuously updated as the input $X_t$ evolves over time. Thus, neural CDE is a continuous analogue of the recurrent neural network. Finally, let $l_\theta:\mathbb{R}^w\rightarrow\mathbb{R}^p$ be a linear map mapping the hidden states $Z_t$ to the output $Y_t$, where $p$ is the dimension of the output $Y_t$. The difference between a neural network and a linear map is that a neural network contains an activation function, and a linear map does not. The figure below illustrates the difference between neural ODE and neural CDE in terms of modelling time series data. For a detailed discussion of the comparison between two models, we refer to \cite{kidger2020}.
