...
Note that the points in the graph on the right-hand side (ODE network) represent the evaluation points. These points can be taken according to a sequence of different time stamps $(t_i)_{i=0,\dots,N}$. Then the values given by the model at those points are $z(t_i)_{i=0,\dots,N}$. They can be obtained by solving an ODE:
...
There are a number of methods that do so with varying computational costs. The conventional ideas brought forward by divided differences tend to incur large errors in computation, making gradient calculations inaccurate and thus unreliable. Calculating symbolic expressions of gradients, on the other hand, tends to be rather memory and time consuming. The computational cost of such a method is further compounded by the fact that functions derived from neural networks tend to be rather complicated in nature \cite{tucker2011}. A method that has proven to be time and (relatively) memory efficient is \textit{automatic differentiation} (AD). 

There are two types of AD. \textit{Forward-mode AD} computes the derivative of the function along with the function evaluation. A familiar (albeit limited) concept that implements this is the commutative ring of \textit{dual number} $\mathbb{D}$ over $\mathbb{R}$ generated by $1$ and $\epsilon$, $\epsilon^2 = 0$. \textit{Reverse-mode AD}, and in particular \textit{backpropagation}, first evaluates the function before propagating the derivatives from the output to the input. The paper aims to discuss the implementations of backpropagation in DO (where the backpropagation conducted is discrete) in this section and OD in Section \ref{sec:contback}.

Given a discretised neural ODE (as typically found in the Discretise-then-Optimise method), backpropagation can be used to derive the gradients of $\mathcal{L}(\mathbf{y}, \mathbf{\hat{y}})$ with respect to each parameter in $\theta$. Suppose that we calculate $\hat{y_i}$ (and thus $\mathcal{L}$) via $g(\theta) = (g_m \circ g_{m+1} \circ \cdots \circ g_1) (\theta)$. Additionally, suppose that we know the value of $\frac{\partial \mathcal{L}}{\partial \hat{y_i}}$ - this 
...
By using the result of \ref{eq3} to \ref{eq4}, we can try to find the value for $\dot{a}(t)$:
\begin{align}
...
Since the loss function is a function of $z(t_i)_{i=1,\dots,N}$, we know the exact value of $a(t)$ at these timestamps when performing backpropagation. These values can be obtained either directly (if the neural ODE is the last layer of the network) or as the results from the backpropagation of the next layer (if the neural ODE layer is in the middle of the network). So we can transform the problem into $N$ initial value problems (IVP) as
\begin{align}{\label{eq5}}
    \dot{a}(t) &= -a(t)\frac{\partial f(z(t),\theta,t)}{\partial z(t)},\notag\\
    a(t_i) &= \frac{\partial\mathcal{L}}{\partial z(t_i)},\quad i=1,\dots,N. \tag{5}
\end{align}
...
\subsubsection{Gradients wrt. \(\theta\) and \(t\)}
...
In this case, a neural ODE model involves several steps. First, we load the MNIST dataset and preprocess it in the same way as before. We then define a neural network that will serve as the right-hand side of the neural ODE, thereby creating a neural ODE layer. We then define the loss function and train the model with an optimiser over multiple epochs. Finally, we evaluate the performance of the trained model and compare it with what we did before.
