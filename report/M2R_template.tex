...
Financial time series data, such as stock prices and market indices, exhibit continuous dynamics that can be challenging to model accurately. Traditional machine learning models often struggle to capture these dynamics due to their discrete nature. Neural Ordinary Differential Equations (Neural ODEs) provide a powerful framework for modeling the continuous evolution of market data by integrating neural network architectures within differential equation solvers. This approach leverages the flexibility of neural networks to capture complex patterns while maintaining the mathematical rigor of differential equations.
...
\]
Here, \(f\) is a neural network with parameters \(\theta\), which captures the underlying dynamics of the financial time series data. By embedding the neural network within the ODE framework, Neural ODEs can naturally handle the continuous evolution of market data. This is particularly advantageous for modeling financial time series, where capturing the precise timing and value of price changes is crucial.

\subsection{Numerical Methods for Solving ODEs}
Runge-Kutta methods are numerical techniques used to solve ordinary differential equations (ODEs). These methods (especially RK4) are widely used due to their balance of accuracy and computational efficiency. In the context of Neural ODEs, Runge-Kutta methods help in approximating the continuous evolution of the hidden states over time. By discretizing the time domain and iteratively updating the state variables, these methods ensure stable and accurate solutions, which are essential for reliable financial modeling.
...
The loss function in machine learning quantifies the difference between the predicted values and the actual market data. For financial time series, common choices include Mean Squared Error (MSE) or more sophisticated loss functions that account for financial risk metrics. The loss function drives the optimization process, guiding the model to learn accurate representations of the market dynamics. Gradient descent is the optimization algorithm used to minimize the loss function. In Neural ODEs, the adjoint sensitivity method is employed to efficiently compute gradients with respect to the model parameters, facilitating back-propagation through the ODE solver. Gradient descent updates the parameters \(\theta\) to reduce the loss, iteratively improving the model's performance.


\subsection{Approaches to Optimization}
...
Also, note that the backpropagation through CDE is similar to that of ODE. In particular, the discretise-then-optimise method is exactly the same as that of ODE, and the optimise-then-discretise method is similar to that of ODE. Since the proofs are very similar, we omit the proof here.
