...
We shall formalise this idea in this section. In order to represent the dependence of a given $\theta_t$, we rewrite the loss function as $\mathcal{L}(\mathbf{y},\mathbf{\hat{y}}) = \mathcal{L}(\theta_t)$. The simplest gradient descent method can be described as the following iterative process, given an initial guess $\theta_0$:
$$
...
There are several conditions to be fulfilled for the gradient descent algorithm to be
