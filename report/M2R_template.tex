...
Consider a neural ODE defined by neural networks $f_\theta$, such that $f_\theta$, when provided with parameters $\theta$, defines the differential equation of the system $\dot{x} = f_\theta\left(t, x\right)$. There are two predominant methods to solve Neural ODEs, known as Discretise-then-Optimise (DO) and Optimise-then-Discretise (OD). These methods will be elaborately described later on.
...
The first numerical method deals with solving 
points to cover:
\begin{itemize}
    \item motivation for euler's method
    \item definition of euler's method
    \item limitations of euler's method
    \item general runge-kutta methods definition (including butcher's tableau)
    ...
    \item a short comment on what makes a good method for solving ODEs - discuss implicit runge-kutta methods
\end{itemize}
