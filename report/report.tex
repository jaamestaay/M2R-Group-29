...
In the first section, we describe some foundational notions of neural networks. We review the history and basic structure of neural networks, including key concepts such as layers, activation functions, and loss functions. Optimisation strategies are also discussed, specifically gradient descent and automatic differentiation for efficient calculations. We then explore some simple applications to further illustrate these ideas and help us to discover how neural networks work in terms of real-world scenarios, such as regression and classification problems.
...
Furthermore, we provide two applications for neural ODEs. We first extend the MNIST handwritten digit classification task from classic neural networks to neural ODEs, improving the accuracy from $80\%$ to around $95\%$ with even faster running time and a smaller number of parameters, thereby demonstrating the capabilities of neural ODEs. \textcolor{red}{We then provide another application of using neural ODEs using a regression optimisation problem.}

Finally, building on the framework of neural ODEs to handle more complex and irregular data streams, we extend our discussion to neural controlled differential equations (CDEs). We delve into the definition of neural CDEs and discuss and apply methods to model time series data using neural CDEs and a technique from numerical analysis called cubic splines.
...
A sample image from MNIST is shown below. We immediately see that it is a $6$. We want our model to recognise it as a $6$ as well, i.e., that the model thinks that the probability that this digit is a $6$ is higher than any other digit.
...
We  write equation (\ref{eq1}) in the form of equation (\ref{eq2}), where $f$ is a function that depends on the state of layer $t$ and some parameters $\theta_t$ related to this layer. Then, by transforming equation (\ref{eq2}), we get
...
\subsection{Regression with Neural ODEs}

In this section, we return our attention to the example we considered in Section \ref{sec:regex}, in which we implemented nonlinear regression with $y = \sin x$. With the help of neural ODEs, we shall implement a better version of this. The ideas in this section are taken from \cite{nguyen2020exploration}, and we use part of the Julia code from it. The file containing all the code below can be found in the project's GitHub repository at \verb|Neural_ODE.ipynb|.

\subsection{Classification with Neural ODEs}
...
Before we introduce neural CDEs, we first explore the idea of a \textit{recurrent neural network} (RNN). In fact, so far, we have been discussing \textit{feedforward neural networks} (FNNs) only, whose flow of information is uni-directional, from the input layer to the output layer, without any cycles or loops. FNNs are sufficient for ordinary tasks such as regression and classification. However, for more complicated problems, RNNs play an important role. In RNNs, the output of previous steps can be fed as input to the current step, allowing it to maintain a form of ‘memory’. This makes RNNs particularly effective for tasks such as time series prediction, natural language processing, and speech recognition.
